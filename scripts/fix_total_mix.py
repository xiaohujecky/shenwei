...
def searchMix(infile, outfile, bin_start, bin_end):
    ...
    sub_keys_sort = sorted(sub_keys, key=lambda x:int(x))
        ...
        for p_item in  itm_keys_sort :
            ...
            for p_bin in bin_keys_sort :
                ...
                if p_sub not in record[p_bin] or p_item not in record[p_bin][p_sub]:
                    ...
                    for placeholder in info_stack_temp :
                        total_lines += 1
                        #print "WARNING-add: {}".format(placeholder)
                        #print(placeholder)
    ...
    print "Removed 0 value lines : {}".format(dump_lines)
    fout.close()
